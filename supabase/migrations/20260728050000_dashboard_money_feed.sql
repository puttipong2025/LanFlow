-- Keep the live money feed independent from the cached Dashboard cards.
-- Keyset pagination keeps the query cost bounded as historical data grows.
  ...
  return (
    ...
    financial_events as (
      ...
      select
        ...
        'รับซื้อยางจากใบชั่ง — '
          || coalesce(nullif(ot.customer_name, ''), 'ไม่ระบุลูกค้า'),
        'expense',
        ot.total_amount,
        ot.created_by_name
...
revoke all on function public.get_dashboard_money_feed(
  uuid,
  timestamptz,
  text,
  integer
) from public, anon;
grant execute on function public.get_dashboard_money_feed(
  uuid,
  timestamptz,
  text,
  integer
