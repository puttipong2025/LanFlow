-- Reject non-array attendance payloads before any monthly replacement.
-- SQL NULL makes "<> 'array'" evaluate to unknown, so use IS DISTINCT FROM.
...
begin
  ...
  if p_month !~ '^[0-9]{4}-[0-9]{2}$' or jsonb_typeof(p_selections) is distinct from 'array'
