...
alter table public.rubber_bills
  add column if not exists source_rubber_export_id uuid references public.rubber_exports(id),
  add column if not exists source_export_no text,
  add column if not exists received_at timestamptz,
  add column if not exists received_age_hours numeric(14,6),
  add column if not exists received_age_is_estimated boolean;

alter table public.rubber_bills drop constraint if exists rubber_bills_branch_receipt_shape_check;
...
create unique index if not exists rubber_bills_one_active_branch_receipt
  on public.rubber_bills(source_rubber_export_id)
  where source_rubber_export_id is not null and record_status = 'active';

create index if not exists rubber_bills_branch_receipt_source_lookup
  on public.rubber_bills(source_rubber_export_id, record_status);

alter table public.rubber_export_items
  add column if not exists carried_age_hours numeric(14,6);

alter table public.rubber_export_items drop constraint if exists rubber_export_items_carried_age_check;
...
drop trigger if exists guard_branch_receipt_bill on public.rubber_bills;
...
drop trigger if exists guard_received_rubber_export_delete on public.rubber_exports;
...
drop function if exists public.get_rubber_export_age_summaries(uuid);
