...
function detailLine(line: WexDetails["lines"][number]): DraftLine {
  return {
    ...
    outboundAt: bangkokDateTimeInput(new Date(line.outboundAt)),
...
export function ExportVehicleWeighBillFormModal({
  ...
  const lineNetWeights = useMemo(() => lines.map((line) => {
    ...
    return Number.isFinite(inbound) && Number.isFinite(outbound) ? outbound - inbound : 0;
  }), [lines]);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      if (!inboundAt || !outboundAt) {
        setError(`กรุณาระบุเวลาเข้าและออกของรถคันที่ ${index + 1}`);
        return;
      }
      if (!Number.isFinite(inboundWeight) || !Number.isFinite(outboundWeight) || outboundWeight <= inboundWeight) {
        setError(`น้ำหนักขาออกของรถคันที่ ${index + 1} ต้องมากกว่าน้ำหนักขาเข้า`);
        return;
      }
    ...
    >
      <form id={formId} noValidate onSubmit={submit} className="space-y-5" aria-busy={saving}>
        ...
        <section aria-labelledby="wex-vehicle-lines-title" className="space-y-3">
          <div className="flex flex-wrap items-end justify-between gap-2">
            <div>
              ...
              <p className="text-sm text-ink/60">ทะเบียนแต่ละคันต้องไม่ซ้ำ และน้ำหนักขาออกต้องมากกว่าขาเข้า</p>
            </div>
            {lines.length < 2 && (
            ...
            return (
              <fieldset key={index} className="rounded-lg border border-black/10 bg-field/50 p-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
                  ...
                  <label className="block"><span className="mb-1 block text-sm font-semibold text-ink/70">เวลาออก</span><input required type="datetime-local" value={line.outboundAt} onChange={(event) => updateLine(index, { outboundAt: event.target.value })} className="focus-ring h-11 w-full rounded-md border border-black/15 bg-white px-3" /></label>
                  <div className="flex gap-2"><label className="block min-w-0 flex-1"><span className="mb-1 block text-sm font-semibold text-ink/70">น้ำหนักขาออก</span><input required aria-label={`น้ำหนักขาออกคันที่ ${index + 1}`} type="number" min="0" step="0.01" value={line.outboundWeight} onChange={(event) => updateLine(index, { outboundWeight: event.target.value })} className="focus-ring h-11 w-full rounded-md border border-black/15 bg-white px-3 tabular-nums" /></label>{lines.length > 1 && <button type="button" aria-label={`ลบรถคันที่ ${index + 1}`} onClick={() => setLines((current) => current.filter((_, lineIndex) => lineIndex !== index))} className="focus-ring mt-6 inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-actionSecondary text-white"><Minus size={16} aria-hidden="true" /></button>}</div>
                </div>
                <p className={netWeight > 0 ? "mt-2 text-sm font-semibold text-leaf" : "mt-2 text-sm font-semibold text-ink/55"}>น้ำหนักสุทธิรถคันนี้: {formatExportVehicleWeighBillNumber(Math.max(netWeight, 0))} กก.</p>
              </fieldset>
            );
          })}
        </section>
        ...
        <section aria-labelledby="wex-rex-title" className="space-y-3">
          <div><h3 id="wex-rex-title" className="font-bold text-ink">จองรายการ REX ที่ขายออกแล้ว</h3><p className="text-sm text-ink/60">เลือกได้ 0 รายการขึ้นไป; ระบบ Server ตรวจสิทธิ์ สาขา การจองซ้ำ และน้ำหนักอีกครั้ง</p></div>
