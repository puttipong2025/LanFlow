...
export function CashBranchTransferCreateModal({ location, transfer, online, modeSelector, onSave, onClose }: { location: Location; transfer?: CashBranchTransfer; online: boolean; modeSelector?: React.ReactNode; onSave: (payload: unknown) => Promise<unknown>; onClose: () => void }) {
  ...
  return <Modal title={transfer ? "แก้ไขการโยกเงินสด" : "โยกเงินไปสาขาอื่น (เงินสด)"} modeSelector={modeSelector} onClose={onClose}>
    ...
    <footer className="flex flex-col-reverse gap-2 border-t border-black/[0.07] pt-4 sm:flex-row sm:items-center sm:justify-end">
      <button type="button" onClick={onClose} className="focus-ring rounded-xl bg-actionSecondary px-4 py-2.5 font-semibold text-white transition hover:bg-actionSecondary/90">ยกเลิก</button>
      <button disabled={saving || !online} onClick={() => void submit()} className="focus-ring inline-flex items-center justify-center gap-2 rounded-xl bg-commit px-5 py-2.5 font-bold text-white shadow-[0_8px_18px_rgba(61,115,163,0.24)] transition hover:bg-commit/90 disabled:cursor-not-allowed disabled:opacity-40"><Save size={17} />{saving ? "กำลังบันทึก..." : "บันทึกรายการ"}</button>
    </footer>
  </Modal>;
}
...
export function CashBranchTransferReceiveModal({ transfer, online, onReceive, onClose }: { transfer: CashBranchTransfer; online: boolean; onReceive: (counts: CashDenominationCounts) => Promise<unknown>; onClose: () => void }) {
  ...
  return <Modal title="ตรวจรับเงินสด" onClose={onClose}>
    ...
    <p className={difference === null || difference === 0 ? "text-success" : "font-bold text-clay"}>
      ผลต่างรวม: {difference === null ? "กรอกข้อมูลให้ครบ" : formatCurrency(difference)}
    </p>
    <footer className="flex flex-col-reverse gap-2 border-t border-black/[0.07] pt-4 sm:flex-row sm:items-center sm:justify-end">
      <button
        type="button"
        onClick={onClose}
        className="focus-ring rounded-xl bg-actionSecondary px-4 py-2.5 font-semibold text-white transition hover:bg-actionSecondary/90"
      >
        ยกเลิก
      </button>
      <button
        ...
        onClick={() => void submit()}
        className="focus-ring inline-flex items-center justify-center gap-2 rounded-xl bg-commit px-5 py-2.5 font-bold text-white shadow-[0_8px_18px_rgba(61,115,163,0.24)] transition hover:bg-commit/90 disabled:cursor-not-allowed disabled:opacity-40"
      >
        <Save size={17} />
