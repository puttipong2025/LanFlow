...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error("RPC create_stock_product_approval_request error:", error);
      return NextResponse.json({ status: "failed", errorMessage: error.message }, { status: 500 });
    }

    const result = (data || {}) as StockProductApprovalRpcResponse;
    ...
    console.error("Create stock product approval request API error:", message);
