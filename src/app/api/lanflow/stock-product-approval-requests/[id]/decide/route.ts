...
export async function POST(
  ...
  try {
    ...
    const { data, error } = await adminCheck.supabase.rpc(
      "decide_stock_product_approval_request",
      {
        p_request_id: id,
        p_decision: decision,
    ...
    if (error) {
      console.error("RPC decide_stock_product_approval_request error:", error);
      return NextResponse.json({ status: "failed", errorMessage: error.message }, { status: 500 });
    }

    const result = (data || {}) as StockProductDecisionRpcResponse;
    ...
    console.error("Decide stock product approval request API error:", message);
