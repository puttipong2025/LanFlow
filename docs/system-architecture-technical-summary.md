...
### Income/Expense Derived Money-Transfer Rows

นอกจาก row จริงใน `income_expense` แล้ว โมดูลรายรับ-รายจ่ายยังแสดงรายการที่ derive มาจาก `money_transfers` เพื่อให้เงินที่เกิดจากโมดูลโอนเงินสะท้อนในตารางรับ-จ่ายโดยไม่สร้างข้อมูลซ้ำ:
...
2. **รายจ่ายส่วนสาขาจ่ายจากโอนเงินลูกค้า**
...
- derived money-transfer rows ถูกล็อกเสมอและแก้/ลบจากรับ-จ่ายไม่ได้

### Module Coverage

สถานะ offline-first ณ ตอนนี้:

- **Rubber Bills**: Full Offline สำหรับ create/update/delete, PWA reload, RPC atomic sync
...
- **Money Transfer**: online-first; เป็น source ของ derived rows ใน Income/Expense สำหรับโอนเงินสาขาขาเข้าและโอนลูกค้าสถานะ `โอน+สาขาจ่าย`
...
| Derived money-transfer rows | ไม่มี | แสดงรายรับจาก `transfer_type='branch'` และรายจ่ายจาก `transfer_status='branch_and_transfer'` โดยล็อกแก้/ลบ |
...
- derived row ในรับ-จ่ายต้อง disable edit/delete พร้อมข้อความไทยชัดเจน และต้องแก้จากโมดูลโอนเงินต้นทางเท่านั้น
